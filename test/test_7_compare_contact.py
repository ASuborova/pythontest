...
def test_compare_contact(app):
    ...
    all_contact = app.cont_h.get_list_contact_join()
    ...
    contact_fio_from_view_page = app.cont_h.join_get_contact_fio_info_from_view_page(index_compare)

    assert contact_fio_from_view_page.fio_home == fio_on_home_page(contact_fio_from_view_page)

    assert contact_from_view_page.phones_view == all_phones_on_home_page(contact_from_edit_page)
...
def fio_on_home_page(contact):
    return "\n".join(filter(lambda x: x != "", map(lambda x: clean(x), filter(lambda x: x is not None, [contact.firstname, contact.lastname, contact.middlename]))))
